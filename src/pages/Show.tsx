    ...
    return (
      <div className="bg-[#0f0e0d] min-h-screen text-white">
        ...
        <div className="flex items-center justify-center h-96">
          <p className="text-[#9a9590] text-sm animate-pulse">
            Loading show...
          </p>
        </div>
      </div>
    ...
    <div className="bg-[#0f0e0d] min-h-screen text-white">
      ...
      <div className="relative h-[240px] overflow-hidden bg-[#1a1815]">
        ...
        <div className="absolute bottom-0 left-0 right-0 px-8 pb-6 flex flex-col items-center text-center">
          ...
          {/* 2. Japanese Title */}
          {show.titleJapanese && (
            <p className="text-[11px] text-[#5a5650] mb-3">
              {show.titleJapanese}
            </p>
          )}
          ...
          <div className="flex items-center justify-center gap-2 mb-3 flex-wrap">
            {show.genres.slice(0, 3).map((g) => (
              <span
                key={g}
                className="text-[10px] text-[#D13924] bg-[#D13924]/10 border border-[#D13924]/25 px-2 py-0.5 rounded-full"
              >
                {g}
              </span>
            ))}
        ...
        <div className="flex gap-6 items-start">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            <div className="bg-[#1a1815] border border-white/7 rounded-xl p-5">
              <h2 className="text-[13px] font-medium text-[#f0ede8] mb-3">
                Synopsis
              </h2>
              {show.synopsis ? (
                <p className="text-[13px] text-[#c8c4be] leading-relaxed">
                  {cleanSynopsis(show.synopsis)}
                </p>
              ) : (
            ...
            {activeTab === "episodes" && (
              <div className="bg-[#1a1815] border border-white/7 rounded-xl p-5">
                {currentPageEpisodes.length === 0 && !loadingEpisodes ? (
                  <p className="text-[13px] text-[#5a5650] text-center py-6">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {totalEpisodePages > 1 && (
                          ...
                          >
                            {Array.from(
                              { length: totalEpisodePages },
                              (_, i) => {
                                const start = i * 100 + 1;
                                const isLastPage = i === totalEpisodePages - 1;
                                const end =
                                  isLastPage && show.airing
                                    ? "Current"
                                    : (i + 1) * 100;
                                return (
                        ...
                        {displayedEpisodes.map((ep) => {
                          ...
                          return (
                            <div
                              key={ep.number}
                              onClick={() =>
                                (window.location.href = `/show/${show.id}/episode/${ep.number}`)
                              }
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-[12px] text-[#f0ede8] truncate">
                                  {ep.title}
                                </div>
                                {ep.airDate && (
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                {ep.filler && (
                                  <span className="text-[9px] text-[#9a9590] bg-white/5 px-2 py-0.5 rounded">
                                    Filler
                                  </span>
                                )}
                                {ep.recap && (
                                ...
                                <span className="text-[10px] text-[#D13924]">
                                  View →
                                </span>
                              </div>
                            </div>
            ...
            {activeTab === "discussions" && (
              <div className="bg-[#1a1815] border border-white/7 rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-[13px] font-medium text-[#f0ede8]">
                    Community discussions
                  </h2>
                  {isLoggedIn && (
                    <button
                      onClick={() =>
                        (window.location.href = `/thread/new?showId=${show.id}&showName=${encodeURIComponent(show.title)}`)
                      }
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[10px] text-[#9a9590] mb-1">
                              {disc.threadType === "episode" &&
                                `S${disc.season} Ep ${disc.episode}`}
                              {disc.threadType === "season" &&
                                `Season ${disc.season}`}
                              {disc.threadType === "show" &&
                                "General discussion"}
                            </div>
                            <div className="text-[13px] font-medium text-[#f0ede8]">
                              {disc.threadTitle}
                            </div>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <div className="flex flex-col gap-6">
                    {show.related.map((group) => (
                      <div key={group.relation}>
                        <h3 className="text-[12px] font-medium text-[#9a9590] mb-3 uppercase tracking-wider">
                          {group.relation}
                        </h3>
                        <div className="flex flex-col gap-2">
                          {group.entries.map((entry) => (
                            <div
                              ...
                              onClick={() =>
                                entry.type === "anime" &&
                                (window.location.href = `/show/${entry.id}`)
                              }
                              className={`flex items-center justify-between p-3 rounded-lg border border-white/5 transition-all ${
                                entry.type === "anime"
                                  ? "cursor-pointer hover:border-[#D13924]/30"
                                  : "cursor-default"
                              }`}
                            >
                              <div>
                                <div className="text-[12px] text-[#f0ede8]">
                                  {entry.title}
                                </div>
                                <div className="text-[10px] text-[#9a9590] mt-0.5 capitalize">
                                  {entry.type}
                                </div>
                              </div>
                              {entry.type === "anime" && (
                                <span className="text-[11px] text-[#D13924]">
                                  View →
                                </span>
                              )}
                            </div>
            ...
            {(show.openingThemes.length > 0 ||
              ...
              <div className="bg-[#1a1815] border border-white/7 rounded-xl p-5">
                <h2 className="text-[13px] font-medium text-[#f0ede8] mb-4">
                  Music
                </h2>
                {show.openingThemes.length > 0 && (
                  <div className="mb-4">
                    <div className="text-[11px] text-[#9a9590] mb-2 uppercase tracking-wider">
                      Opening
                    </div>
                    {show.openingThemes.map((theme, i) => (
                      <div
                        key={i}
                        className="text-[12px] text-[#c8c4be] py-2 border-b border-white/5 last:border-0"
                      >
                        {theme}
                      </div>
                    ))}
          ...
          <div className="w-[260px] flex-shrink-0 flex flex-col gap-4">
            ...
            <div className="bg-[#1a1815] border border-white/7 rounded-xl p-5">
              ...
              <div className="flex flex-col gap-3">
                {[
                  ...
                  {
                    label: "Rating",
                    value: show.rating?.split(" - ")[0] || "Unknown",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
                  ...
                  >
                    <span className="text-[11px] text-[#9a9590] flex-shrink-0">
                      {item.label}
                    </span>
                    <span className="text-[11px] text-[#f0ede8] text-right">
                      {item.value}
                    </span>
                  </div>
                ))}
            ...
            <div className="bg-[#1a1815] border border-white/7 rounded-xl p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {[...show.genres, ...show.themes, ...show.demographics].map(
                  (g) => (
                    <span
                      key={g}
                      className="text-[10px] text-[#D13924] bg-[#D13924]/10 border border-[#D13924]/25 px-2 py-0.5 rounded-full"
